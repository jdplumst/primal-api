        ...
        public InfoDto(int pageNumber, int pageSize, int totalSize)
        {
            ...
            if (pageNumber > 1)
            {
                PreviousPage = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development"
                    ? Environment.GetEnvironmentVariable("DEV_SERVER_URL") + "/size?pageNumber=" + (pageNumber - 1) + "&pageSize=" + pageSize
                    : Environment.GetEnvironmentVariable("PROD_SERVER_URL") + "/size?pageNumber=" + (pageNumber - 1) + "&pageSize=" + pageSize;
            }
