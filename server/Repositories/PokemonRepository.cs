...
namespace PrimalAPI.Repositories
{
    public class PokemonRepository : IPokemonRepository
    {
        ...
        {
            var sizes = _context
                .Size.Where(s => s.Name.ToLower() == sizeName.ToLower())
                .Select(p => p.Id);
        ...
        {
            return _context
                .Pokemon.Where(p => p.Habitat.Any(h => h.Id == habitatId))
                .OrderBy(p => p.Id)
                .ToList();
        }
        ...
        {
            return _context
                .Pokemon.Where(p => p.Habitat.Any(h => h.Name.ToLower() == habitatName.ToLower()))
                .OrderBy(p => p.Id)
                .ToList();
        }
        ...
        {
            return _context.Pokemon.Where(p => p.Skill.Any(s => s.Id == skillId)).ToList();
        }
        ...
        {
            return _context
                .Pokemon.Where(p => p.EggGroup.Any(e => e.Name.ToLower() == eggGroupName.ToLower()))
                .OrderBy(p => Guid.NewGuid())
                .FirstOrDefault()!;
        }
    }
